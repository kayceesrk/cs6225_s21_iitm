...
Proof.
   ...
   *   hypothesis. *)
  assert (exists v', 
       eval (v $+ ("output", out * S n) $+ ("input", n)) factorial_loop v'
    /\ v' $? "output" = Some (fact n * (out * S n))).
  apply IHn.
  simplify; equality.
  simplify; equality.
  first_order.
  (* [first_order]: simplify first-order logic structure.  
   * 
   * Be forewarned: this one is especially likely to run 
   * forever! 
   * 
   * In this case, same effect as [invert H1. invert H2] *)
...
Lemma small_big'' : forall v c v' c', 
  ...
  -> forall v'', eval v' c' v''
                 -> eval v c v''.
Proof.
  (* Proof is straight-forward.. skipping.. *)
  induct 1; simplify.
...
Definition trsys_of (v : valuation) (c : cmd) 
           : trsys (valuation * cmd) := 
{| Initial := {(v, c)};
   Step := step |}.

Theorem simple_invariant :
  invariantFor (trsys_of ($0 $+ ("a", 1)) 
                         ("b" <- "a" + 1;; "c" <- "b" + "b"))
               (fun s => snd s = Skip -> fst s $? "c" = Some 4).
Proof.
  model_check. 
  (* We have not see [model_check]. But that's ok. *)
Qed.
...
(* [Skip] is a member of [all_programs] set *)
Example ex1 : all_programs Skip. 
Proof.
  unfold all_programs.
  simplify.
  equality.
...
Lemma manually_proved_invariant' : forall n, isEven n -> 
  invariantFor 
    (trsys_of ($0 $+ ("n", n) $+ ("a", 0)) 
              (while "n" loop 
                 "a" <- "a" + "n";; 
    ...
    (fun s => all_programs (snd s) (* set membership test *)
...
Proof.
  ...
  subst; simplify.
  exists n, 0.
  (* instantiate existential variables with values *)
  propositional.
  constructor.
...
Lemma manually_proved_invariant'_snazzy : forall n, isEven n -> 
  invariantFor 
    (trsys_of ($0 $+ ("n", n) $+ ("a", 0)) 
              (while "n" loop 
                 "a" <- "a" + "n";; 
    ...
    (fun s => all_programs (snd s) (* set membership test *)
              /\ exists n a, fst s $? "n" = Some n
                          ...
                          /\ isEven a).
Proof.
  simplify; apply invariant_induction; simplify; 
  unfold all_programs in *; first_order; subst; simplify;
  try match goal with
...
Theorem manually_proved_invariant : forall n, isEven n -> 
  invariantFor 
    (trsys_of ($0 $+ ("n", n) $+ ("a", 0)) 
              (while "n" loop 
                 "a" <- "a" + "n";; 
...
(* We'll return to these systems and their abstractions 
...
(* We can prove equivalence between contextual small 
   step semantics and small-step semantics.
   See [OperationalSemantics.v] *)
...
Module Concurrent.
  ...
  Proof.
    ...
    econstructor.
    eapply CStep with (C := CPar1 (CSeq Hole _) _).
    econstructor.
    econstructor.
    econstructor.
    econstructor.
